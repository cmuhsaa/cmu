...
export default async function TeacherPage({ searchParams }) {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Faculty Directory</h1>
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                      <div>
                        ...
                        <p className="text-gray-800">{member.email || 'No email'}</p>
                        <p className="text-gray-800">{member.phone || 'No phone'}</p>
                      </div>
                      
                      <div>
                        <p className="text-sm text-gray-500">Address</p>
                        <p className="text-gray-800">{member.address || 'No address'}</p>
                      </div>
                    </div>
                    ...
                    {member.about && (
                      <div className="mt-4">
                        ...
                        <p className="text-gray-800 line-clamp-3">{member.about}</p>
                      </div>
                    )}
                    
                    <div className="flex justify-between mt-4 text-sm text-gray-500">
                      <span>Joined: {member.createDate?.date} at {member.createDate?.formatedTime}</span>
                      <span>Updated: {member.updateDate?.date} at {member.updateDate?.formatedTime}</span>
                    </div>
                  </div>
                </div>
              </div>
